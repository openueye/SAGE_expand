...
from .prepared_scene_contract import PREPARED_SCENE_V2, validate_v2_source_contract
...
class PreparedScene:
    ...
    def _manifest_schema(self) -> str | None:
        try:
            payload = json.loads(self.prepared_manifest_path.read_text(encoding="utf-8"))
    ...
    def _frame_input_paths(self, stem: str) -> tuple[tuple[str, Path], ...]:
        center_label = "center_depth" if self._manifest_schema() == PREPARED_SCENE_V2 else "raw_depth"
        return (
            (f"rgb/{stem}", self.config.scene_dir / "images" / f"{stem}.png"),
            (f"{center_label}/{stem}", self.config.center_depth_path / f"{stem}.npy"),
    ...
    def frames(self, *, limit: int = -1) -> Iterator[FrameInputs]:
        ...
        if manifest["schema_version"] == PREPARED_SCENE_V2:
            source = manifest["source"]
            assert isinstance(source, dict)
            center_source_type = SourceType[source["center_source_type"]]
            fused_source_type = SourceType[source["fused_source_type"]]
        else:
            center_source_type = SourceType.LIDAR_RAW
            fused_source_type = SourceType.LIDAR_FUSED5
        ...
        for index, (stem, association) in enumerate(ordered):
            ...
            center_label = "center_depth" if manifest["schema_version"] == PREPARED_SCENE_V2 else "raw_depth"
            raw_path = frame_paths[f"{center_label}/{stem}"]
            fused_path = frame_paths[f"fused5_depth/{stem}"]
            fused_mask_path = frame_paths[f"fused5_mask/{stem}"]
            for label, path in (("RGB image", image_path), ("raw depth", raw_path), ("fused5 depth", fused_path), ("fused5 mask", fused_mask_path)):
                if not path.is_file():
                    raise ValueError(f"Missing {label} for frame {stem}: {path}")
            with Image.open(image_path) as image:
                rgb = np.asarray(image.convert("RGB"), dtype=np.float32) / 255.0
            raw_depth_loaded = np.load(raw_path)
            fused_depth_loaded = np.load(fused_path)
            if raw_depth_loaded.dtype != np.dtype("<f4") or fused_depth_loaded.dtype != np.dtype("<f4"):
                raise ValueError(f"Prepared Scene depth must be little-endian float32: {stem}")
            raw_depth = np.asarray(raw_depth_loaded, dtype=np.float32)
            ...
            if rgb.shape != (*native_shape, 3) or raw_depth.shape != native_shape or fused_depth.shape != native_shape or fused_mask_raw.shape != native_shape:
            ...
            if manifest["schema_version"] == PREPARED_SCENE_V2 and (
                not np.isfinite(raw_depth).all() or not np.isfinite(fused_depth).all()
                or (raw_depth < 0).any() or (fused_depth < 0).any()
            ):
            ...
            if manifest["schema_version"] == PREPARED_SCENE_V2:
                if not np.array_equal(fused_mask_raw.astype(bool), fused_depth > 0):
                    raise ValueError(f"Prepared Scene v2 fused mask does not match fused depth: {stem}")
                if raw_depth[raw_valid].tobytes() != fused_depth[raw_valid].tobytes():
                    raise ValueError(f"Prepared Scene v2 center anchor is not byte-exact in fused depth: {stem}")
            ...
            if self.config.resize_width is not None:
                ...
                raw_depth = _resize_nearest(raw_depth, output_size, dtype=np.float32)
                raw_valid = _resize_nearest(raw_valid, output_size, dtype=np.float32).astype(bool)
            ...
            mapping_depth = np.where(raw_valid, raw_depth, np.where(fused_valid, fused_depth, 0.0)).astype(np.float32)
            mapping_sources = np.full(mapping_depth.shape, INVALID_SOURCE_TYPE, dtype=np.uint8)
            mapping_sources[raw_valid] = int(center_source_type)
            mapping_sources[~raw_valid & fused_valid] = int(fused_source_type)
            ...
            mapping_confidence[raw_valid] = raw_confidence
            mapping_confidence[~raw_valid & fused_valid] = fused_confidence
            evidences = [DepthEvidence(
                center_source_type, raw_depth, raw_valid,
                np.where(raw_valid, raw_confidence, 0.0).astype(np.float32),
            )]
            if fused5_enabled:
    ...
    def _validate_contract(self) -> dict[str, object]:
        ...
        if manifest.get("schema_version") == PREPARED_SCENE_V2:
            return self._validate_v2_contract(manifest)
        raise ValueError(f"Prepared Scene must use {PREPARED_SCENE_V2}")

    def _validate_v2_contract(self, manifest: dict[str, object]) -> dict[str, object]:
        expected_fields = {
        ...
        contract = validate_v2_source_contract(manifest)
